...
double Software::Analyze::ProbingSecurity(Software::SettingsStruct& Settings,  std::vector<Software::SharedDataStruct>& SharedInputData, Software::HelperStruct& GlobalHelper, std::vector<Software::ThreadSimulationStruct>& GlobalThreadSimulations){
    ...
    TableCell time_cell = logger.GetTimeCell(EndClock(begin), strlen("Elapsed Time"));
    TableCell memory_cell = logger.GetMemoryCell(GetUsedMemory(), strlen("Used Memory"));

    std::string processed_simulations;
    if (Settings.CompactDistributions){
		processed_simulations = std::to_string(NumberOfProcessedSimulations);
    }else{
		processed_simulations = std::to_string(NumberOfProcessedSimulations) + " / 10000000000";
	}

    std::string maximum = "STORE_MEMSHADOW[31](" + std::to_string(Settings.Max_No_ClockCycles) + "), ";
    std::vector<TableCell> header{
        ...
        {"Probing Set with highest Information Leakage", maximum.size() * Settings.TestOrder},
    ...
    for(StepSimulationIndex = 0; StepSimulationIndex < (Settings.NumberOfSimulations / Settings.NumberOfStepSimulations); ++StepSimulationIndex){
        ...
        Software::Print::SoftwareMaximumProbingSet(Settings.TestOrder, GlobalTests.at(std::get<0>(MaximumProbingSet)), std::get<1>(MaximumProbingSet), maximum);
        double p_value = GlobalTests.at(std::get<0>(MaximumProbingSet)).GlobalProbingSets.at(std::get<1>(MaximumProbingSet)).contingency_table_.GetLog10pValue();

        if (Settings.CompactDistributions){
		    processed_simulations = std::to_string(NumberOfProcessedSimulations);
        }else{
			processed_simulations = std::to_string(NumberOfProcessedSimulations) + " / " + std::to_string(ComputeRequiredSampleSize(Settings.NumberOfGroups, GlobalTests.at(std::get<0>(LargestContingencyTableProbingSet)).GlobalProbingSets.at(std::get<1>(LargestContingencyTableProbingSet)).contingency_table_.GetNumberOfEntries(), Settings.BetaThreshold, Settings.EffectSize));
		}

        time_cell = logger.GetTimeCell(EndClock(begin), strlen("Elapsed Time"));
        memory_cell = logger.GetMemoryCell(GetUsedMemory(), strlen("Used Memory"));
        std::string status = (p_value < 5.0) ? "OKAY" : "LEAKAGE";
        std::vector<TableCell> body{
            {time_cell.content_, header[0].width_},
            {memory_cell.content_, header[1].width_},
            {std::to_string(GlobalProbingSetSize), header[2].width_},
            {processed_simulations, header[3].width_},
            {maximum, header[4].width_},
            {std::to_string(p_value), header[5].width_},
            {status, header[6].width_}
        };
