...
TruthTable::TruthTable(uint64_t amount_of_inputs) {
  amount_of_inputs_ = amount_of_inputs;
  table_ = std::vector<std::vector<bool>>(1 << amount_of_inputs,
                                          std::vector<bool>(2));
}

TruthTable::TruthTable(const std::vector<bool>& init) {
  assert((init.size() & (init.size() - 1)) == 0 &&
         "Error in TruthTable::TruthTable: init size is not a power of 2!");
  amount_of_inputs_ = std::log2(init.size());
  table_.reserve(init.size());

  for (bool bit : init) {
    table_.push_back(std::vector<bool>{bit, false});
  }
}
...
std::vector<T> TruthTable::MutateVectorOrder(std::vector<T> const& inputs) {
  if (inputs.size() % 3 != 0) {
    throw std::runtime_error(
        "propagation library expression has amount of inputs not divisible by "
        "3 : ");
  }
  ...
  for (uint8_t input_index = 0; input_index < (inputs.size() / 3);
       ++input_index) {
    for (auto type_index : type_order) {
      mutated_inputs[mutated_index] =
          inputs[type_index * (inputs.size() / 3) + input_index];
      ++mutated_index;
    }
  }
...
template std::vector<uint64_t> TruthTable::MutateVectorOrder<uint64_t>(
    std::vector<uint64_t> const& inputs);
template std::vector<std::string> TruthTable::MutateVectorOrder<std::string>(
    std::vector<std::string> const& inputs);

Expression TruthTable::OperationFromTruthTable(bool for_glitch) const {
  if (GetAmountOfInputs() > 26)
    throw std::runtime_error(
        "Truthtable doesn't support more than the alhpabet");
  std::vector<std::string> inputs(GetAmountOfInputs());
  char character;
  for (uint64_t i = 0; i < GetAmountOfInputs(); ++i) {
    character = 65 + i;
    inputs[i] = character;
  }
  ...
  for (uint64_t i = 0; i < (1ULL << GetAmountOfInputs()); ++i) {
    ...
    if (!value) continue;
    for (uint64_t input_index = 0; input_index < inputs.size(); ++input_index) {
...
std::string TruthTable::ConstructExpression(std::string op,
...
void TruthTable::PrintTable() const {
  for (uint row_index = 0; row_index < (1U << amount_of_inputs_); ++row_index) {
    ...
    std::cout << ": "
              << std::to_string(static_cast<uint8_t>(table_[row_index][0]))
              << " "
              << std::to_string(static_cast<uint8_t>(table_[row_index][1]))
              << "\n";
  }
}
...
std::vector<bool> const& TruthTable::GetConstRowReference(
...
uint64_t TruthTable::GetAmountOfInputs() const { return amount_of_inputs_; }
