...
Enabler::Enabler(const CellStruct* cell, uint64_t output_index,
                 unsigned int extended_probe_index,
                 std::vector<std::unique_ptr<uint64_t[]>*> input_addresses)
    : cell_(cell),
      output_index_(output_index),
      extended_probe_index_(extended_probe_index),
      input_addresses_(input_addresses) {}

unsigned int Enabler::GetExtendedProbeIndex() { return extended_probe_index_; }

void Enabler::SetInputAddress(unsigned int index,
                              std::unique_ptr<uint64_t[]>* ptr) {
  input_addresses_[index] = ptr;
}

uint64_t Enabler::EvaluateGlitch(std::vector<uint64_t>& input_values) {
  return cell_->EvalGlitch(output_index_, input_values);
}

uint64_t Enabler::EvaluatePropagation(std::vector<uint64_t>& input_values) {
  return cell_->EvalProp(output_index_, input_values);
}

bool Enabler::operator==(const Enabler& other) const {
