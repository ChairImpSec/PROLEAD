...
Fault::Fault(const SignalStruct* signal, uint64_t signal_index,
...
uint64_t Fault::GetFaultedSignalIndex() const { return this->signal_index_; }
uint64_t Fault::GetFaultedClockCycle() const { return this->clock_cycle_; }
...
bool Fault::IsSignalFaulted(uint64_t signal_index, uint64_t clock_cycle) const {
  return (signal_index == this->signal_index_) &&
         (clock_cycle == this->clock_cycle_);
}
