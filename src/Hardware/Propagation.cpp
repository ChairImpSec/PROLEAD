...
template <>
Propagation<RobustProbe>::Propagation(uint64_t signal_index)
    : signal_index_(signal_index) {}

template <>
Propagation<RelaxedProbe>::Propagation(uint64_t signal_index)
    : signal_index_(signal_index) {
...
template <class ExtensionContainer>
Propagation<ExtensionContainer>::Propagation(
    uint64_t signal_index, std::vector<ExtensionContainer> extension_indies)
    : signal_index_(signal_index), extension_indices_(extension_indies) {}

template Propagation<RobustProbe>::Propagation(uint64_t,
                                               std::vector<RobustProbe>);
template Propagation<RelaxedProbe>::Propagation(uint64_t,
                                                std::vector<RelaxedProbe>);

template <>
Propagation<RelaxedProbe>::Propagation(
    uint64_t signal_index, uint64_t enable_index,
    uint64_t number_of_enable_indices, uint64_t number_of_signal_indices,
...
template <>
bool Propagation<RobustProbe>::operator==(
    const Propagation<RobustProbe>& other) const {
  return (signal_index_ == other.signal_index_) &&
         (extension_indices_ == other.extension_indices_);
}

template <>
bool Propagation<RelaxedProbe>::operator==(
    const Propagation<RelaxedProbe>& other) const {
  return (signal_index_ == other.signal_index_) &&
         (extension_indices_[0].enable_index_ ==
          other.extension_indices_[0].enable_index_) &&
         (extension_indices_[0].number_of_enable_indices_ ==
          other.extension_indices_[0].number_of_enable_indices_) &&
         (extension_indices_[0].number_of_signal_indices_ ==
          other.extension_indices_[0].number_of_signal_indices_) &&
         (extension_indices_[0].propagation_indices_ ==
          other.extension_indices_[0].propagation_indices_) &&
         (extension_indices_[0].signal_indices_ ==
          other.extension_indices_[0].signal_indices_);
}
...
template <>
std::vector<uint64_t> Propagation<RelaxedProbe>::GetExtensionIndices(
  ...
  while (!indices.empty()) {
    ...
    std::vector<Propagation>::iterator it =
        std::lower_bound(propagations.begin(), propagations.end(), index,
                         [](Propagation& propagation, uint64_t value) {
    ...
    for (uint64_t propagation_index :
         propagations[index].extension_indices_[0].propagation_indices_) {
      if (considered_indices.find(propagation_index) ==
...
template <>
std::vector<uint64_t> Propagation<RelaxedProbe>::GetEnableIndices(
    std::vector<Propagation>& propagations) {
  std::vector<uint64_t> result, indices, signal_indices;
  std::set<uint64_t> considered_indices;
  uint64_t index;
  ...
  while (!indices.empty()) {
    ...
    if (propagations[index].extension_indices_[0].number_of_enable_indices_) {
      if (std::find(result.begin(), result.end(),
                    propagations[index].extension_indices_[0].enable_index_) ==
          result.end()) {
        result.push_back(
            propagations[index].extension_indices_[0].enable_index_);
      }
...
template <>
void Propagation<RobustProbe>::Propagate(
    const Library& library, const CircuitStruct& circuit,
    std::vector<Propagation>& propagations) {
  uint64_t index, input_index;

  if (circuit.signals_[signal_index_].is_extension_allowed) {
    for (index = 0; index < circuit.GetNumberOfInputsForSignalsComputingCell(
                                signal_index_);
         ++index) {
      input_index =
          circuit.cells_[circuit.signals_[signal_index_].Output].Inputs[index];

      if (propagations[input_index].extension_indices_.empty()) {
        propagations[input_index].Propagate(library, circuit, propagations);
      }

      extension_indices_.insert(
          extension_indices_.end(),
          propagations[input_index].extension_indices_.begin(),
          propagations[input_index].extension_indices_.end());
    }
    ...
    if (circuit.signals_[signal_index_].is_analysis_allowed) {
...
template <>
void Propagation<RelaxedProbe>::Propagate(
    const Library& library, const CircuitStruct& circuit,
    std::vector<Propagation>& propagations) {
  uint64_t index, input_index;
  uint64_t signal_index = BackpropagateUntilBranch(circuit, signal_index_);

  if (circuit.signals_[signal_index].is_extension_allowed) {
    extension_indices_[0].enable_index_ = signal_index;
    ++extension_indices_[0].number_of_enable_indices_;

    for (index = 0;
         index < circuit.GetNumberOfInputsForSignalsComputingCell(signal_index);
         ++index) {
      input_index =
          circuit.cells_[circuit.signals_[signal_index].Output].Inputs[index];
      input_index = BackpropagateUntilBranch(circuit, input_index);

      if (propagations[input_index]
              .extension_indices_[0]
              .signal_indices_.empty()) {
        propagations[input_index].Propagate(library, circuit, propagations);
      }
      ...
      if (!propagations[input_index]
               ...
               .signal_indices_.empty()) {
        extension_indices_[0].number_of_enable_indices_ +=
            propagations[input_index]
                .extension_indices_[0]
                .number_of_enable_indices_;
        extension_indices_[0].number_of_signal_indices_ +=
            propagations[input_index]
                .extension_indices_[0]
                .number_of_signal_indices_;
        extension_indices_[0].propagation_indices_.push_back(input_index);
      }
    }
  }

  if (circuit.signals_[signal_index].is_analysis_allowed) {
...
template <>
bool Propagation<RobustProbe>::IsObsolete(const CircuitStruct& circuit,
                                          const Settings& settings) {
  uint64_t signal_index = GetSignalIndex();
  if (!circuit.signals_[signal_index].is_probe_allowed) {
    return true;
  }
  ...
  if (settings.GetMinimization() != Minimization::none) {
    for (int index = 0; index < circuit.signals_[signal_index].NumberOfInputs;
         ++index) {
      if (circuit.cells_[circuit.signals_[signal_index].Inputs[index]]
              .type->GetType() != cell_t::sequential) {
        return true;
      }
    }
  }
...
template <>
bool Propagation<RelaxedProbe>::IsObsolete(const CircuitStruct& circuit,
                                           const Settings& /*settings*/) {
  uint64_t signal_index = GetSignalIndex();
...
template <>
Propagation<RelaxedProbe> Propagation<RelaxedProbe>::ExtendWithTime(
    uint64_t clock_cycle, std::vector<Probe>& probes,
  ...
  if (extension_indices_[0].number_of_enable_indices_) {
    ...
    propagation.extension_indices_[0].enable_index_ =
        SearchEnabler(*probe_index, enabler);
  }

  propagation.extension_indices_[0].number_of_enable_indices_ =
      extension_indices_[0].number_of_enable_indices_;
...
template <>
void Propagation<RelaxedProbe>::Finalize(
  ...
  for (uint64_t& index : extension_indices_[0].propagation_indices_) {
    it = std::lower_bound(
        propagations.begin(), propagations.end(), index,
...
template <>
void Propagation<RelaxedProbe>::UpdateNumberOfSignals(
    std::vector<Propagation<RelaxedProbe>>& propagations) {
  extension_indices_[0].number_of_signal_indices_ =
      GetExtensionIndices(propagations).size();
  extension_indices_[0].number_of_enable_indices_ =
      GetEnableIndices(propagations).size();
}
}  // namespace Hardware
