...
void RunDomIndepD1FaultyRand(const std::string& configfile, const std::string& expected_probe_report_name) {
    std::string path = "tests/full/prolead/combined/dom_indep_d1_faulty_rand/";
    po::variables_map vm;
...
bool CheckDomIndepD1FaultyRand(bool expected_leakage) {
  std::string leakage_report_name;

  uint64_t idx = 0;
  leakage_report_name = "tests/full/prolead/combined/dom_indep_d1_faulty_rand/"
  ...
  while (std::filesystem::exists(leakage_report_name)) {
    ...
    leakage_report_name = "tests/full/prolead/combined/dom_indep_d1_faulty_rand/prolead_dom_indep_d1_faulty_rand_" 
...
BOOST_AUTO_TEST_SUITE(test_dom_indep_d1_faulty_rand)

BOOST_AUTO_TEST_CASE(t1, *utf::label("dom_indep_d1") *utf::label("dom") *utf::label("d1")
  *utf::label("combined") *utf::label("faulty rand")) {
  RunDomIndepD1FaultyRand("faulty_rand.json", "faulty_rand_expected_probes.json");
  BOOST_CHECK(CheckDomIndepD1FaultyRand(true));

  uint64_t idx = 0;
  std::string leakage_report_name = "tests/full/prolead/combined/dom_indep_d1_faulty_rand/"
    "prolead_dom_indep_d1_" + std::to_string(idx) + "_0_16384_leakages.json";
  while (std::filesystem::exists(leakage_report_name)) {
    std::filesystem::remove(leakage_report_name);
    leakage_report_name = "tests/full/prolead/combined/dom_indep_d1_faulty_rand/prolead_dom_indep_d1_" 
      + std::to_string(++idx) + "_0_16384_leakages.json";
  }
}
