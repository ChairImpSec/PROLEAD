 ...
 */
 ...
 /**
  * @details This function can also be replaced by a buil-in function from the
  * std or boost library. However, the current implementation guarantees that the
  * function early aborts as soon as a difference is found between the two keys.
  * This is beneficial for performance reasons.
  */
 int64_t CompareEntries(const TableEntry& lhs, const TableEntry& rhs, uint64_t size_of_keys_in_bytes);
 
 bool MergeEntries(const TableEntry& lhs, const TableEntry& rhs, uint64_t size_of_keys_in_bytes, uint64_t number_of_groups);                    
 ...
 BucketContainer::iterator SortAndMergeDuplicates(BucketContainer& observations,
 ...
 void UpdateBucketWithBucket(BucketContainer& bucket,
                             ...
                             typename BucketContainer::iterator observations_end,
                             uint64_t size_of_key_in_bytes,
                             uint64_t number_of_groups);
 ...
 class ContingencyTable {
  public:
    ...
    * mode.
    */
   void Initialize(uint64_t number_of_probes, uint64_t number_of_groups,
   ...
   void SetLog10pValue(double_t log_10_p_value) {
     log_10_p_value_ = log_10_p_value;
   }
  ...
  //private:
   ...
   void CompressCounters(std::vector<uint32_t>& counters,
