...
class ProbingSet {
 public:
  ...
  /**
   * @brief Initializes the probing set with probes and their corresponding extensions.
   *
   * This method sets the probes and their corresponding extensions for the probing set.
   *
   * @param probe_addresses A vector of pointers to the probes placed by the adversary.
   * @param probe_extension_indices A vector of indices representing the resulting probe extensions.
   */
   ...
   *
   * This method returns a vector containing the addresses of all standard probes within the probing set.
   *
 ...
 *
 * This method calculates the (maximum) number of probe extensions based on the 
 * probe type specialization. For `RobustProbe`, it returns the size of the `probe_extension_indices_` 
 * vector, as each index corresponds to a distinct extension. For `RelaxedProbe`, the method computes 
 * the total number of extensions by summing the signal and enable indices from the associated 
 * `Propagation` objects.
 *
 * @param propagations A vector of `Propagation` objects, which are used to calculate the total 
 * number of extensions for `RelaxedProbe` specializations. This parameter is unused for `RobustProbe`.
 * @return The total maximum number of probe extensions.
 * @tparam ExtensionContainer The type of the probe, either `RobustProbe` or `RelaxedProbe`.
 * @note The method's behavior differs based on the specialization: 
 * - For `RobustProbe`, it returns the count of elements in `probe_extension_indices_`.
 * - For `RelaxedProbe`, it aggregates the number of signal and enable indices.
 * @see Propagation
 */
   ...
   *
   * This method returns the index of a specific extended probe based on its position 
   * in the probe_extension_indices_ vector.
   *
   * @param extended_probe_index The index of the extended probe in the probe_extension_indices_ vector.
   ...
   *
   * Since the probe_extension_indices_ list is sorted, this method returns the first 
   * probe extension, which is typically used for analyzing the smallest clock cycle 
   * among the extensions.
   *
   * @return The index of the first extended probe in the extended_probes_ vector.
   * @see Adversaries
   */
   ...
   *
   * Since the probe_extension_indices_ list is sorted, this method returns the last 
   * probe extension, which is typically used for analyzing the highest clock cycle 
   * among the extensions.
   *
   * @return The index of the last extended probe in the extended_probes_ vector.
   * @see Adversaries
   */
   ...
   *
   * This method returns a list of all unique probe extensions in the probing set.
   *
   ...
   *
   * This method flags the probing set as removable, indicating that it is strictly 
   * less informative compared to another set and can be discarded.
   * 
   * @note This is typically used in optimization procedures to reduce the number of probing sets.
   */
...
/**
 * @brief Checks whether the current probing set fully includes another probing set.
 *
 * This method determines if the current `ProbingSet` (referred to as "this") fully includes another
 * `ProbingSet` (referred to as "other"). A probing set is said to fully include another if all the probe 
 * extensions in "other" are also present in "this", and the set of extensions in "this" spans a wider or equal
 * range of clock cycles than those in "other".
 *
 * The inclusion check follows these steps:
 * 1. If either the current probing set or the other set is marked as removable, they cannot include one another.
 * 2. If the number of probe extensions in "this" is less than or equal to that in "other", "this" cannot include "other".
 * 3. If the range of clock cycles covered by "this" (from the first to the last probe extension) is wider or equal to 
 *    that of "other", the method proceeds to the next step.
 * 4. Finally, if all probe extensions of "other" are found within "this", in the correct order, the method returns true, 
 *    indicating that "this" fully includes "other". Otherwise, it returns false.
 *
 * @param other The other `ProbingSet` to check for inclusion within the current set.
 * @param propagations A vector of `Propagation` objects used to retrieve probe extension information.
 * @return True if the current probing set fully includes the other set, false otherwise.
 * @tparam ExtensionContainer The type of the probe extension, typically `RobustProbe` or `RelaxedProbe`.
 * @note This method ensures that "this" must have a wider or equal range of probe extensions and include all 
 *       probe extensions of the "other" set in order to return true.
 * @see GetNumberOfProbeExtensions, GetFirstProbeExtension, GetLastProbeExtension, IsRemovable, Propagation
 */
   ...
   * distributions of the observation set. The initialization can vary based on whether 
   * the system is operating in compact mode.
   *
   * @param is_in_compact_mode A boolean indicating whether PROLEAD operates in compact mode.
   * @param propagations A vector of Propagation objects containing the propagation procedures.
   */
  void Initialize(bool is_in_compact_mode,
                  std::vector<Propagation<ExtensionContainer>>& propagations, uint64_t number_of_groups);

  /**
   * @brief Retrieves the value of -log10(p).
   *
   * This method returns the value of -log10(p), which is used in statistical analysis.
   *
