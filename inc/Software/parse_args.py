...
def extract_args(FolderName):    
    ...
    armV = None
    out,err = run("arm-none-eabi-readelf -A " + RELATIVE_PATH + "tmp_data/binary.elf")
    tmp,err = run("arm-none-eabi-readelf -A " + RELATIVE_PATH + "tmp_data/binary.elf")
    ...
    with open(RELATIVE_PATH + "tmp_data/binary.map", "rt") as file:
    ...
    begin_ignore = None
    sections = list()
    out,err = run("arm-none-eabi-objdump " + RELATIVE_PATH + "tmp_data/binary.elf -t")
    ...
    sections = list()
    out,err = run("arm-none-eabi-objdump " + RELATIVE_PATH + "tmp_data/binary.elf -h")
    ...
    for name, _ in sections:
        run("arm-none-eabi-objcopy -O binary --only-section="+name+" " + RELATIVE_PATH + "tmp_data/binary.elf " + RELATIVE_PATH + "tmp_data/code_section"+name)
    ...
    with open(RELATIVE_PATH + "tmp_data/disassembled.txt", "r") as f:
        lines = f.readlines()
        sanitised_lines = []
        for line in lines:
    ...
    for name, offset in sections:
        args += " --section " + RELATIVE_PATH + "tmp_data/code_section{} {}".format(name, offset)
